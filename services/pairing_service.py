...
class PairingService:
    ...
    def get_pairing_counts(self, member_ids: list[int]) -> dict[tuple[int, int], dict]:
        ...
        normalized_ids = sorted(set(int(mid) for mid in member_ids))
        ...
        with self.db.get_conn() as conn:
            rows = conn.execute(
                ...
                WHERE member_a_id IN ({placeholders})
                  AND member_b_id IN ({placeholders})
                """,
        ...
        for row in rows:
            key = (
                min(int(row["member_a_id"]), int(row["member_b_id"])),
                max(int(row["member_a_id"]), int(row["member_b_id"])),
            )
            result[key] = dict(row)
    ...
    def pairing_penalty(
        ...
        for other_id in group_member_ids:
            key = (
                min(candidate_member_id, other_id),
                max(candidate_member_id, other_id),
            )
    ...
    def update_history_for_outing(self, outing_id: int) -> None:
        with self.db.get_conn() as conn:
            ...
            for row in rows:
                ...
                grouped.setdefault(row["tee_time_id"], []).append(int(row["member_id"]))
                ...
                for i in range(len(group)):
                    for j in range(i + 1, len(group)):
                        ...
                        if existing:
                            conn.execute(
                                """
                                UPDATE pairing_history
                                SET times_paired = ?, last_paired_date = ?
                                WHERE id = ?
                                """,
                                (
                                    existing["times_paired"] + 1,
                                    outing_date,
                                    existing["id"],
                                ),
                            )
